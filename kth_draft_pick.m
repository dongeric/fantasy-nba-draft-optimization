function [pick, x, y, z] = kth_draft_pick(N, k, num_picks, pick_number, teams, selected_players, remaining_players)
...
[num_players, ~] = size(selected_players);
opp_players = 1:n;
ranks = remaining_players(:, 3);
non_opp_players = [ranks; selected_players.'];
...
for k_bar = k+1:num_picks
    for i = 1:n
        ...
        if ismember(i, ranks)
            if find(ranks == i) <= alpha * (nk_bar - nk)
                A(row, n * 21 + i) = 1;
            end            
        end
    end
...
picks = y;
if ~isempty(selected_players)
    picks(selected_players) = 0;
end
pick = find(picks, 1);
